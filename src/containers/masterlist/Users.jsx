  ...
  const handleToggleExport = async () => {
    ...
    const newObj = await data?.data?.map((item) => {
      ...
      return {
        ...
        Role: item?.role,
        Department: item?.department?.label || item?.department,
        "Sub Unit": item?.sub_unit?.label || item?.sub_unit,
        Location: item?.location?.label || item?.location,
        Division: item?.division?.label || item?.division,
        "Division Category":
